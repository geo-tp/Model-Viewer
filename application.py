...
def load_data(name):
    with urllib.request.urlopen(f"https://geo-tp.github.io/Model-Viewer/static/{name}") as url:
...
def show_dirs():
    if not check_maintenance():
        if request.path == '/creatures':
            images = creatures_images
            session['in_view'] = "creatures"
        elif request.path == '/items':
            images = items_images
            session['in_view'] = "items"
...
def show_dir(dir):
    if not check_maintenance():
        ...
        dirs.sort()
        return render_template(template, images=images_in_dir, dir=dir, dirs=dirs)
    else:
        return render_template('maintenance.html')
...
@app.route("/update")
def update():
    items_images = load_data("items_images.json")
    creatures_images = load_data("creatures_images.json")
    return redirect("/")
